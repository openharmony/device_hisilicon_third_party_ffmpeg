...
static int track_header(VividasDemuxContext *viv, AVFormatContext *s,  uint8_t *buf, int size)
{
    ...
    for(i=0;i<viv->num_audio;i++) {
        ...
        if (avio_tell(pb) < off) {
            int num_data;
            int xd_size = 1; // fix CVE-2020-35964
            int data_len[256];
            int offset = 1;
            uint8_t *p;
            ...
            for (j = 0; j < num_data; j++) {
                ...
                data_len[j] = len;
                xd_size += len + 1 + len/255; // fix CVE-2020-35964
            }

            ret = ff_alloc_extradata(st->codecpar, xd_size); // fix CVE-2020-35964
            if (ret < 0)
                return ret;
            ...
            for (j = 0; j < num_data - 1; j++) {
                ...
                av_assert0(delta <= xd_size - offset); // fix CVE-2020-35964
                offset += delta;
            }
            ...
            for (j = 0; j < num_data; j++) {
                ...
                av_assert0(data_len[j] <= xd_size - offset); // fix CVE-2020-35964
                offset += data_len[j];
            }
